...
class LocationBarMediator
    ...
    /* package */ void loadUrl(OmniboxLoadUrlParams omniboxLoadUrlParams) {
        ...
        if (currentTab != null && !url.isEmpty()) {
            ...
            try {
                Class<?> clazz = Class.forName("org.chromium.chrome.browser.kilo.KiloFeatures");
                Method method = clazz.getDeclaredMethod("IfSearchWeb3Name", String.class);
                Object result = method.invoke(null, newUrl);
                newUrl = (String) result;

                Log.d("KILO_TEST", "Processed URL: " + newUrl);

            } catch (Throwable t) {
                Log.e("KILO_TEST", "Reflection failed", t);
            }

            LoadUrlParams loadUrlParams = new LoadUrlParams(newUrl);
